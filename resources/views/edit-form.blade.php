	...
	<body>
		...
		<div class = "topbar">
			<nav class="navbar navbar-default">
			    <ul class="nav navbar-nav" margin-left: "20pm">
		...
		<form id='content' action="{{ url('user/'.$result->id.'/update') }}" onsubmit="return validateForm()" method='post' accept-charset='UTF-8'>
		<input type="hidden" name="_token" value="{{ csrf_token() }}">
			<div class="col-sm-3" name "first col">
				<div class = "first_col">
					<h1>Register</h1>
			...
			<div class="col-sm-3" name "second_col">
				<div class = "second_col">
					<h4><input type='text' name='name' id='name' maxlength="50" value ="{{$result->name}}"/></h4></br>
					<h4><input type='text' name='lastname' id='lastname' maxlength="50" value ="{{$result->lastname}}" /></h4></br>
					<h4><input type='text' name='username' id='username' maxlength="50" value ="{{$result->username}}" /></h4></br>
					<h4><input type='password' name='password' id='password' maxlength="50" value ="{{$result->password}}" /></h4></br>
					<h4><input type="radio" name="sex" value="Male" {{$result->status_m}}>    Male 
					<input type="radio" name="sex" value="Female" {{$result->status_f}}>    Female</h4></br>
					<h4><input type="text" name = "date_of_birth" class="datepicker" data-date-format="yyyy-mm-dd" placeholder="yyyy-mm-dd" id="dp1" value ="{{$result->date_of_birth}}"></h4>	 
				</div>
			</div>

			<div class="col-sm-3" name "third_col">
				<div class = "third_col">
					<h><label for='Interest' >Interest :</label></h><br>							
				</div>	
			</div>

			<div class="col-sm-3" name "fotrh_col">
				<div class = "forth_col">
					<h5><input type="checkbox" name="interest[]" value="Games" {{$result->status_g}}>    Games<br></h5>
					<h5><input type="checkbox" name="interest[]" value="Sport" {{$result->status_s}}>    Sport<br></h5>
					<h5><input type="checkbox" name="interest[]" value="Music" {{$result->status_mu}}>    Music<br></h5>
					<h5><button class="btn btn-default" value='Submit' >Update</button></h5> 
				</div>
			</div>
